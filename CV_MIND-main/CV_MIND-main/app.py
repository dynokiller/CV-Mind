...
def analytics():
    ...
    skill_freq = {}
    for a in activities:
        # Use strengths or skills field
        skills = a.get("strengths", []) or a.get("skills", [])
        for skill in skills:
    ...
    analytics_data = {
        ...
        "skill_values": [skill_freq[s] for s in sorted(skill_freq, key=skill_freq.get, reverse=True)[:10]],
...
def settings():
    if not is_logged_in():
        return redirect(url_for("signin"))
    user = safe_find_one(user_collection, {"user_id": session["user_id"]}) or {}
...
def change_password():
    ...
    user = safe_find_one(user_collection, {"user_id": session["user_id"]})
    if not user:
        return jsonify({"status": "error", "message": "User not found."}), 404

    # Google-login users have no password hash
    ...
    if not (
        ...
        return jsonify({"status": "error", "message": "Password must be 6-15 chars with 1 uppercase, 1 number, and 1 special char (@.&-_)."}), 400
